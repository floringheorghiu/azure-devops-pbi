      ...
      fc.assert(fc.property(
        fc.record({
          ...
          acceptanceCriteria: fc.array(
            fc.string({ maxLength: 100 }).map(s => s + '<b>Bold</b> & special chars'),
            { maxLength: 5 }
          )
        }),
        ({ title, description, acceptanceCriteria }) => {
          const pbiData: PBIData = {
            id: 12345,
            title,
            state: 'Active',
            description,
    ...
    test('should truncate long content appropriately for display modes', () => {
      fc.assert(fc.property(
        fc.record({
          ...
          manyAcceptanceCriteria: fc.array(
...
function renderPBIData(pbiData: PBIData, displayMode: 'compact' | 'expanded') {
  return {
    id: pbiData.id,
    title: pbiData.title,
    state: pbiData.state,
